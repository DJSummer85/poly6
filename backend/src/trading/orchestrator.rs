...
pub async fn restore_running_bots(orchestrator: Arc<BotOrchestrator>) {
    ...
    let running_sessions = match queries::get_all_running_sessions(&orchestrator.db).await {
        Ok(s) => s,
        Err(e) => { tracing::error!("Failed to fetch running sessions: {}", e); return; }
    };
    if running_sessions.is_empty() { tracing::info!("No running bots to restore"); return; }
    ...
    for session in &running_sessions {
        let bot = match queries::get_bot_by_id(&orchestrator.db, session.bot_id, session.user_id).await {
            Ok(Some(b)) => b,
            Ok(None) => { tracing::warn!("Bot {} not found, skipping", session.bot_id); continue; }
            Err(e) => { tracing::warn!("Failed to fetch bot {}: {}", session.bot_id, e); continue; }
        };
        let running_bot = RunningBot {
            bot_id: bot.id, session_id: session.id, user_id: session.user_id,
            strategy: StrategyExecutor::new(&bot.strategy_type, &bot.params),
            last_market_slug: None, consecutive_errors: 0, last_btc_price: None,
            btc_window_open: None, current_balance: session.start_balance,
            pending_bet: None, btc_price_history: Vec::new(),
        };
        orchestrator.running_bots.write().await.insert(bot.id, running_bot);
        tracing::info!("Restored bot {} (session {})", bot.id, session.id);
        let orchestrator_clone = orchestrator.clone();
        let bot_id = bot.id;
        let session_user_id = session.user_id;
        tokio::spawn(async move {
            start_orchestrator_loop(orchestrator_clone, bot_id, session_user_id, 5, None).await;
        });
    }
    tracing::info!("Finished restoring running bots");
}
...
impl BotOrchestrator {
    ...
    pub async fn resume_bot(&self, bot: &BotRecord, current_balance: f64) -> Result<i64, String> {
        ...
        running.insert(bot.id, RunningBot {
            bot_id: bot.id, session_id, user_id: bot.user_id, strategy,
            last_market_slug: None, consecutive_errors: 0, last_btc_price: None,
            btc_window_open: None, current_balance, pending_bet: None, btc_price_history: Vec::new(),
        });
    ...
    pub async fn start_bot(&self, bot: &BotRecord, initial_balance: f64) -> Result<i64, String> {
        ...
        let mut running = self.running_bots.write().await;
        running.insert(bot.id, RunningBot {
            bot_id: bot.id, session_id, user_id: bot.user_id,
            strategy: StrategyExecutor::new(&bot.strategy_type, &bot.params),
            last_market_slug: None, consecutive_errors: 0, last_btc_price: None,
            btc_window_open: None, current_balance: initial_balance,
            pending_bet: None, btc_price_history: Vec::new(),
        });
        self.event_sender.send(BotEvent::SessionStarted { bot_id: bot.id, session_id, bot_name: bot.name.clone() }).ok();
    ...
    pub async fn execute_cycle(&self, bot_id: i64, user_id: i64, credential_cache: Option<Arc<RwLock<HashMap<i64, CachedCredentials>>>>) -> Result<(), String> {
        ...
        eprintln!("[DEBUG] Bot {} cycle: balance={:.2}, mode={}", bot_id, portfolio.balance, bot.trading_mode);
        ...
        // BTC változás és velocity számítás
        let btc_change;
        let btc_velocity;
        let btc_acceleration;
        {
            ...
            if rb.btc_price_history.len() >= 2 {
                ...
                // btc_change: tört arány (pl. 0.003 = +0.3%)
                btc_change = Some((btc_price - oldest) / oldest);

                let duration_secs = {
                    let first_t = rb.btc_price_history.first().unwrap().1;
                    let last_t = rb.btc_price_history.last().unwrap().1;
                ...
                if rb.btc_price_history.len() >= 3 {
                    ...
                    let oldest2 = rb.btc_price_history[mid_idx].0;
        ...
        if market_changed || market_ended {
            if let Some(ref bet) = rb.pending_bet {
                ...
                eprintln!("[SETTLE] Bot {}: {} won={} profit={:.4}", bot_id, bet.side, won, profit);
                rb.pending_bet = None;
            }
            if market_changed {
                rb.btc_window_open = Some(btc_price);
                rb.last_market_slug = Some(market_slug.clone());
                tracing::info!("[MARKET] Bot {} new market: {} ({}s remaining)", bot_id, market_slug, market.time_remaining);
            }
        }

        // FIX: StrategyContext helyes mezőnevekkel a strategies.rs alapján
        let ctx = StrategyContext {
            btc_price,                                          // f64 (nem Option!)
            btc_change,                                         // Option<f64>
            btc_window_open: rb.btc_window_open,               // Option<f64>
            yes_price: market.yes_price,                       // f64 (nem Option!)
            no_price: market.no_price,                         // f64 (nem Option!)
            time_remaining: market.time_remaining,             // i64
            btc_velocity,                                       // Option<f64>
            btc_acceleration,                                   // Option<f64>
            btc_volatility: btc_acceleration.map(|a| a.abs()), // Option<f64>
        };
        ...
        if let Signal::Yes(conf) | Signal::No(conf) = signal {
            tracing::info!("Bot {} signal: {:?} (confidence: {:.3})", bot_id, signal, conf);
            ...
            if rb.pending_bet.is_none() {
                ...
                self.event_sender.send(BotEvent::TradeDecision {
                    bot_id, outcome: outcome.to_string(), confidence: conf,
                    bet_size: bot.bet_size, reason: "Signal detected".into(),
                }).ok();
                ...
                if bot.trading_mode == "live" {
                    ...
                    let d_id = queries::log_trade_decision(
                        ...
                        &market_slug, &market.condition_id, outcome,
                        &bot.strategy_type, conf,
                        Some(btc_price), btc_change,
                        Some(market.yes_price), Some(market.no_price),
                        Some(market.time_remaining), "paper trade",
                    ).await.unwrap_or(0);
                    ...
                    rb.pending_bet = Some(PendingBet {
                        ...
                        start_price: btc_price, entry_price: price, decision_id: d_id,
                    });

                    self.event_sender.send(BotEvent::PositionUpdate {
                        bot_id, side: outcome.to_string(), size: bot.bet_size, price, unrealized_pnl: 0.0,
                    }).ok();
                }
            }
        }
        ...
        self.running_bots.write().await.insert(bot_id, rb);
        Ok(())
    }
    ...
    pub async fn get_running_bots(&self, user_id: i64) -> Vec<i64> {
        self.running_bots.read().await.iter().filter(|(_, b)| b.user_id == user_id).map(|(id, _)| *id).collect()
    }
    pub async fn auto_save_sessions(&self) -> Result<(), String> { Ok(()) }
}

pub async fn start_orchestrator_loop(
    orchestrator: Arc<BotOrchestrator>, bot_id: i64, user_id: i64,
    interval_secs: u64, credential_cache: Option<Arc<RwLock<HashMap<i64, CachedCredentials>>>>,
) {
